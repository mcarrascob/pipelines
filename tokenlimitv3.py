...
class Pipeline:
    ...
    async def get_user_info(self, username: str) -> Optional[Dict]:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.api_url}/users/{username}") as response:
                    if response.status == 404:
                        raise Exception(f"Usuario {username} no encontrado en el sistema. Por favor, asegúrate de haber iniciado sesión correctamente o contacta con el soporte para obtener asistencia.")
    ...
    async def use_tokens(self, username: str, tokens: int) -> bool:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.api_url}/users/{username}/consume", json={"tokens": tokens}) as response:
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        if user and user.get("role") in self.valves.target_user_roles:
            ...
            try:
                ...
                user_tokens = user_info.get('tokens', 0)
                
                if user_tokens <= 0:
                    raise Exception("Lo siento, no te quedan tokens. Ponte en contacto con el soporte para obtener más información.")
                
                incoming_tokens = self.count_tokens(body.get('messages', []))
                if incoming_tokens > user_tokens:
                    raise Exception(f"Tu mensaje requiere {incoming_tokens} tokens, pero solo tienes {user_tokens} disponibles. Por favor, acorta tu mensaje o compra más tokens.")
